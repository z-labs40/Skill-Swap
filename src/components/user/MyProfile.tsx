...
export function MyProfile() {
  ...
  const { logout } = useAuth(); // Import useAuth if needed
          ...
          <>
            ...
            <div className="fixed top-[68px] right-4 z-[70] flex flex-col gap-2.5 sm:hidden">
              {[
                ...
                { label: 'Explore',   icon: '🔍', action: () => { setMobileMenuOpen(false); navigate('/user/explore'); }, color: 'border-blue-500/40 bg-blue-950/80 text-blue-200' },
                { label: 'Messages',  icon: '💬', action: () => { setMobileMenuOpen(false); navigate('/user/messages'); }, color: 'border-emerald-500/40 bg-emerald-950/80 text-emerald-200' },
              ].map((item, i) => (
                <motion.button
                  key={item.label}
